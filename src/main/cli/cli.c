...
static void cliTestBitBangDecode(char *cmdline);
...
#endif
    CLI_COMMAND_DEF("bb_decode", "decode test", NULL, cliTestBitBangDecode),
};
...
FAST_RAM_ZERO_INIT uint32_t array[200];

static NOINLINE void cliTestBitBangDecode(char *cmdline)
{
    UNUSED(cmdline);
    ...
    uint32_t values[] = { makeGcrValue(0xfff), makeGcrValue(0xfff), makeGcrValue(0xfff) };
...
//        decodeBitBangTelemetryPacket( array, 200, valuesOut, masks, 3);
        q += decodeBitBangTelemetryPacket2( array, 200, kk);
    }
    cliPrintf("Millis for 1000 x 3 decodes: %u %u\n", millis() - m, q);
#endif    
}
